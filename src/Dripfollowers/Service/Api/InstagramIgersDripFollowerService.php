...
class InstagramIgersDripFollowerService extends InstagramIgersService {
    private $_daily_followers;

    public function __construct($plugin, $delay) {
        parent::__construct ( $plugin );
        $this->_daily_followers = ceil(1/($delay/86400000)); // $delay is ms between each follow
...
*/
		...
		$followers_per_run = 0;
		$_daily_followers = $this->_daily_followers;
		if ($_daily_followers >= 30) {                       // 50
			$delay_between_runs = 86400;               // 1 days
			$followers_per_run = $_daily_followers ;                     // 
		}
		else {
	                 // put bacl june 7 was since 2017  change feb 2019 
	                    	//	$delay_between_runs = ceil(3600*(24/($_daily_followers/100)));     // 25
	                    	$delay_between_runs = ceil(3600*(24/($_daily_followers/100)));     // 25
		             //put bacl june 7  was since 2017 chnge feb 2019       	
		                        $followers_per_run = 100;     // 25
		             //or
	
	   //  change 20 feb 2019 and remove 7 june 	$delay_between_runs = ceil(3600*(24/($_daily_followers/30)));     // 50
		
		 // chnge f20eb 2019  and remove 7 june   	$followers_per_run = 30;     // 50
		}

	//removed &replace20fev 2019
		//$delay_between_runs = 86400;   // new feb 2019
	//removed & reolace 20fev 2019	
	     // $followers_per_run = $_daily_followers ; //		new  feb 2019$delay_between_runs = 86200;
//		$followers_per_run = 10;
	//	$this->_data = 'key=' . $this->_service_access_id . '&action=automatic_add&type=ig_followers&link='.$target.'&amount_total='.$count.'&amount_per_run='.$followers_per_run.'&delay='.$delay_between_runs;
        $this->_data = 'Key=' . $this->_service_access_id . '&ProductId=2&Amount=' .$followers_per_run . '&AmountTotal=' . $count .'&Link=' . $target.'&Pause='.$delay_between_runs;
