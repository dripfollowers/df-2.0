...
abstract class InstagramService {
    ...
    public function __construct(DripFollowers $plugin) {
        $this->_plugin = $plugin;
        $this->_service_url = $plugin->get_setting ( DripFollowersConstants::OPT_INSTAGRAM_SERVICE_URL );
        $this->_service_access_id = $plugin->get_setting ( DripFollowersConstants::OPT_INSTAGRAM_SERVICE_ACCESS_ID );
    }
    ...
    public function send_request($params) {
        try {
            ...
            $result = $this->send ();
            if ($result) {
                return $this->parse_result ( simplexml_load_string ( $result ) );
            }
        } catch ( \Exception $e ) {
    ...
    private function send() {
        ...
        $this->_plugin->get_logger ()->addDebug ( 'InstagramService - Sending Instagram Service Data', array ($log_data) );
        ...
        $result = curl_exec ( $ch );
        curl_close ( $ch );
        $this->_plugin->get_logger ()->addDebug ( 'InstagramService - Instagram Service Response', array ($result) );
