...
class CurlVerifierTest extends PHPUnit_Framework_TestCase {
    ...
    public function testConstructorChecksCurlIsEnabled() {
        ...
        $verifier = $this->getMockBuilder ( 'PayPal\Ipn\Verifier\CurlVerifier' )->setMethods ( array ('curlEnabled' 
        ) )->disableOriginalConstructor ()->getMock ();
        
        $verifier->expects ( $this->any () )->method ( 'curlEnabled' )->will ( $this->returnValue ( false ) );
    ...
    public function testSendVerificationRequest() {
        ...
        $verifier = $this->getMock ( 'PayPal\Ipn\Verifier\CurlVerifier', array ('getRequestUri' 
        ) );
        
        $verifier->expects ( $this->any () )->method ( 'getRequestUri' )->will ( $this->returnValue ( '//INVALID URI' ) );
